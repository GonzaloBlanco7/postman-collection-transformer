...
module.exports = {
    ...
    /**
     * Returns a new collection with each request variable 
     * filled with general values for the entire collection.
     *
     * @param {Object} inputCollection - The initial collection.
     * @returns {Object} - The new collection.
     */
    fillVariables: function (inputCollection) {
        let variableList;
        if (inputCollection.variable) {
            variableList = inputCollection.variable
        } else if (inputCollection.variables) {
            variableList = inputCollection.variables
        } else {
            console.error("no variables to fill");
            return inputCollection;
        }

        let outputString = JSON.stringify(inputCollection);
        variableList.forEach(element => {
            console.error(`variables ${element.key}: ${element.value}`);
            outputString = outputString.replaceAll(`{{${element.key}}}`, element.value);
        });
